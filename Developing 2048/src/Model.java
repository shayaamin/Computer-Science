...
public class Model {
     ...
     *  USE size() tile(int x, int y).
     * */
    public boolean emptySpaceExists() {

        final int size = board.size();
        for (int horiz = 0; horiz < board.size(); horiz++) {
            for (int vert = 0; vert < board.size(); vert++) {
                if (board.tile(horiz, vert) == null) {
                    return true;
                }
            }
        }
    ...
    public boolean maxTileExists() {
        ...
        for (int horiz = 0; horiz < board.size(); horiz++) {
            for (int vert = 0; vert < board.size(); vert++) {
                if (board.tile(horiz, vert) != null && board.tile(horiz, vert).value() == MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int horiz = 0; horiz < board.size(); horiz++) {
            for (int vert = 0; vert < board.size(); vert++) {
                if (horiz < board.size() - 1 && board.tile(horiz + 1, vert) != null
                        && board.tile(horiz, vert).value() == board.tile(horiz + 1, vert).value()) {
                    return true;
                }
                if (vert < (board.size()) - 1 && board.tile(horiz, vert + 1) != null
                        && board.tile(horiz, vert).value() == board.tile(horiz, vert + 1).value()) {
                    return true;
                }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        // More I added.
        for (int value = y + 1; value < board.size(); value++) {
            Tile tileAfter = board.tile(x, value);
            if (tileAfter == null) {
                targetY = value;
            } else if (tileAfter.value() == myValue && !tileAfter.wasMerged() && !currTile.wasMerged()) {
                targetY = value;
                break;
            } else {
                break;
